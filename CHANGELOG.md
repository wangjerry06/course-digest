...
- **实时预览**：输入即更新，只跑 marked。不跑语法高亮与公式渲染 —— 预览是
  「只读视觉确认」，点不动任何东西
...
已知取舍：预览里的代码块没有配色、`$公式$` 显示成源码 —— 要看最终渲染切回查看模式。
（预览与查看若共用同一条渲染链，每次按键都要重跑 highlight.js 与 KaTeX，长文档上会明显卡。）

测试：`tests/frontend_test.js` 由 **172 条增至 333 条**（**+161**），其中编辑器自身
103 条纯函数用例（插入与光标 / 工具栏片段 / 锚点规范化 / 状态条 / 剥回程票 / 计数 / 转义），
其余为接线与样式的静态断言（含「切模式不重建查看区」「预览不挂锚点」两条回归）。
