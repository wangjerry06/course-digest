...
下载件**一定**带票：补票发生在打开页面的本地服务里，而不是只在发布时写一次。因此
**早于本版生成的存量文档也照样带票，不需要重新发布** —— 重新打开页面，再点「下载 MD」
即可。
...
想让 agent **以后所有总结**都按某个风格写？对它说一句就行：

> 「**以后总结要**短一点」「**以后总结**多配例子」「**以后总结**不要拓展块」「**以后总结**用英文」

agent 会把这条记到 `~/.course-digest/style.md`，**之后每次总结都按它写** ——
这个文件跟你的文档库放在一起（不在代码仓库里），所以**换机器、换 agent 也照样生效**。

- 想还原：说「**恢复默认**」
- 只想改这一次：说「**这次**短一点」—— **只对当前这次生效，不会被记下**
